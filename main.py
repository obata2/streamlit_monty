...
プレイヤーは、始めに3つある扉のうち1つを選びます。1つの扉の向こうには賞品である車が(当たり)、残りの2つの扉の向こうにはヤギがいます(外れ)。プレイヤーが扉を選択した後、司会者は外れの扉を1つ開けます。  
**このとき、あなたは最初に選んだ扉をそのまま開けるか、別の扉に変更して開けるかを選択することができます。あなたはどちらを選びますか？**
""")
...
st.write(
  ...
  しかしながら、この問題は計算によって明確な答えを求めることができます。  
  下にその答えと、シミュレーションを示します。
  """)
with st.expander("解説を見る"):
  ...
  **変更する場合**:  
  司会者は必ず外れの扉を1つ開けるため、当たりと外れが1つずつの状態で扉を変更します。最初の時点で選んだ扉が外れであれば、残った当たりの扉に必ず乗り移ることができます。つまり、2/3の確率で当たりをひくことができます。  
  **変更しない場合**:  
  最初の時点で当たりの扉を選んでおくことで、最終的に賞品を手に入れることができます。つまり、1/3の確率で当たりを引くことができます。  
  """)
